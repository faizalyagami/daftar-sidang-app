        ...
        <div class="card-body p-0">
                        ...
                        @forelse($mahasiswas as $index => $mahasiswa)
                            ...
                            <tr>
                                <td class="align-middle">
                                    {{ ($mahasiswas->currentPage() - 1) * $mahasiswas->perPage() + $index + 1 }}</td>
                                ...
                                <td class="align-middle">
                                    @if ($mahasiswa->ipk)
                                        <span
                                            class="badge bg-{{ $mahasiswa->ipk >= 3.0 ? 'success' : ($mahasiswa->ipk >= 2.5 ? 'warning' : 'danger') }}">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="d-flex gap-1">
                                                @foreach ($riwayatSkripsi as $period)
                                                    <span class="badge bg-primary period-badge"
                                                        title="{{ $period }}">{{ $loop->iteration }}</span>
                                                    <span class="badge bg-primary period-badge">{{ $period }}</span>
                                                @endforeach
                                            </div>
                                            <i class="bi bi-info-circle info-icon"
                                                onclick="showPeriods('skripsi', {{ $mahasiswa->id }})"></i>
                                        </div>
                                    @else
                                        -
                                    @endif
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="d-flex gap-1">
                                                @foreach ($riwayatMetodologi as $period)
                                                    <span class="badge bg-success period-badge"
                                                        title="{{ $period }}">{{ $loop->iteration }}</span>
                                                @endforeach
                                            </div>
                                            <i class="bi bi-info-circle info-icon"
                                                onclick="showPeriods('metodologi', {{ $mahasiswa->id }})"></i>
                                        </div>
                                    @else
                                        -
                                    @endif
                                ...
                                <td class="align-middle">
                                    <div class="btn-group" role="group">
                                        <button class="btn btn-sm btn-info" onclick="viewMahasiswa({{ $mahasiswa->id }})"
                                            title="Lihat Detail">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                        <button class="btn btn-sm btn-warning"
                                            onclick="resetPassword({{ $mahasiswa->user->id }})" title="Reset Password">
                                            <i class="bi bi-key"></i>
                                        </button>
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td colspan="13" class="text-center py-5">
                                    ...
                                    @if (request('period_id'))
                                        <p class="text-muted small mt-1">Tidak ada mahasiswa yang terdaftar pada periode
                                            yang dipilih.</p>
                                    @endif
                                </td>
                            </tr>
            ...
            <div class="custom-pagination">
                <div class="pagination-info">
                    ...
                    data
                    @if (request('period_id'))
                        <span class="text-muted"> - Filter periode:
                            {{ $periods->where('id', request('period_id'))->first()->semester ?? '' }}
                            {{ $periods->where('id', request('period_id'))->first()->tahun_akademik ?? '' }}</span>
                    @endif
                </div>
                <div class="d-flex align-items-center">
